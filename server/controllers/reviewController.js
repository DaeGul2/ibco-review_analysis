...
exports.processReviews = async (req, res) => {
  ...
  for (let i = 0; i < reviews.length; i += 10) {
    ...
    const prompt = `
      ...
      3. "해당기관 에대한 지원자의 호감도" (0~5 점수) int
      4. "기존 지원자" (1 혹은 0) 100% 이전에 해당 기관에 지원해본 지원자가 재도전한거면 1, 그렇지 않고 첫 지원이면 0 int
      5. "장점":전체 채용진행에 따라 지원자가 느낀 장점 배열로 나열, 없으면 ['없음']   배열. 카테고리 내에서만 찾되 여러 개 추가 가능, 카테고리 = [면접위원에 대한 의견 / 면접 질문에 대한 의견 / 면접 분위기 / 전형 운영에 대한 의견]
      6. "단점":전체 채용진행에 따라 지원자가 느낀 단점 배열로 나열, 없으면 ['없음']   배열. 카테고리 내에서만 찾되 여러 개 추가 가능, 카테고리 = [면접위원에 대한 의견 / 면접 질문에 대한 의견 / 면접 분위기 / 전형 운영에 대한 의견]
      7. "요청": 리뷰에서 면접 경험에 대해 추가적으로 요청하고 싶은 기능 들을 배열로, 없으면 안 적어도됨
      ...
      {
        "리뷰 데이터 분석": [
          { "리뷰": "리뷰 내용", "판단결과": 숫자(1~5), "비고": "필요할 경우 판단 근거", "호감도": 해당기관 에대한 지원자의 호감도(0~5), "기존지원자": 1or0,"[장점]","[단점]" , "요청": []},
          ...
        ]
      }
