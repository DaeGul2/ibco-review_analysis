...
exports.processReviews = async (req, res) => {
  ...
  for (let i = 0; i < reviews.length; i += 10) {
    ...
    const prompt = `
      ...
      2. "비고" (선택 사항, 판단 근거 설명)
      3. "미래고객가능성" (0~5 점수)
      4. "기존고객" (1 혹은 0) 100% 기존 고객이 또 구매한거면 1, 그렇지 않고 첫 구매같으면 0
      5. "장점":리뷰에서 제품에대한 장점이 있으면 배열로 나열
      6. "단점":리뷰에서 제품에 대한 아쉬운 점이 있으면 배열로 나열, 없으면 안적어도됨됨

      응답 형식 (JSON 객체 내부에 배열을 포함해 출력해야 해):
      {
        "리뷰 데이터 분석": [
          { "리뷰": "리뷰 내용", "판단결과": 숫자(1~5), "비고": "필요할 경우 판단 근거", "미래고객가능성": 숫자(0~5), "기존고객": 1or0,"장점","단점" },
          ...
        ]
      }
