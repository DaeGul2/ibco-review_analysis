...
exports.processReviews = async (req, res) => {
  ...
  for (let i = 0; i < reviews.length; i += 10) {
    ...
    const prompt = `
      ...
      5. "장점":리뷰에서 제품에대한 장점이 있으면 배열로 나열. 단, 카테고리 내의 내용만 추가. 여러 개 추가 가능 카테고리 = [향,성분,발림성,사용감,사용감,순하다,거품,보습]
      6. "단점":리뷰에서 제품에 대한 아쉬운 점이 있으면 배열로 나열, 없으면 안적어도됨. 단, 카테고리 내의 내용만 추가. 여러 개 추가 가능 카테고리 = [향,성분,발림성과 사용감,사용감,건조함,거품]
      7. "요청": 리뷰에서 제품에 대해 추가적으로 요청하고 싶은 기능 들을 배열로, 없으면 안 적어도됨
      8. "비듬": 리뷰에서 비듬관련 언급이 조금이라도 있으면 1 없으면 0
      9. "각질": 리뷰에서 각질관련 언급이 조금이라도 있으면 1 없으면 0
      10. "거품": 리뷰에서 거품관련 언급이 조금이라도 있으면 1 없으면 0

      응답 형식 (JSON 객체 내부에 배열을 포함해 출력해야 해):
      {
        "리뷰 데이터 분석": [
          { "리뷰": "리뷰 내용", "판단결과": 숫자(1~5), "비고": "필요할 경우 판단 근거", "미래고객가능성": 숫자(0~5), "기존고객": 1or0,"장점","단점","요청":["요청사항들"],"비듬":1or0,"각질":1or0,"포장":1or0 },
          ...
        ]
      }
