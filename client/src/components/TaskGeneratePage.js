...
import { generateTask, generateStep2Task } from "../api/taskService"; // ✅ Step2 API 추가
...
const TaskGeneratePage = () => {
  ...
  const handleStep2Submit = async () => {
    ...
    try {
      const prepared = {
        ...step2Inputs,
        sub_competencies: step2Inputs.sub_competencies.split(",").map(s => s.trim())
      };
  ...
  return (
    <div style={{ maxWidth: "900px", margin: "30px auto", padding: "20px", border: "1px solid #ccc", borderRadius: "10px" }}>
      ...
      <section>
        ...
        {step2Result && (
          <div style={{ marginTop: "30px", backgroundColor: "#f8f9fa", padding: "20px", borderRadius: "10px" }}>
            <h5>Step2 결과</h5>
            {Object.entries(step2Result).map(([competency, detail], idx) => (
              <div key={idx} style={{ marginBottom: "30px", padding: "20px", backgroundColor: "white", border: "1px solid #dee2e6", borderRadius: "8px" }}>
                <h6 style={{ marginBottom: "10px" }}>{competency}</h6>
