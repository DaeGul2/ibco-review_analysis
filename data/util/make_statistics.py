...
# 엑셀 파일 불러오기
df = pd.read_excel('한국보건산업진흥원_gpt분석.xlsx')
...
df['기관인상'] = df['기관인상'].apply(clean_and_split)
...
institutionImage = df['기관인상'].explode().reset_index(drop=True).value_counts().reset_index()
institutionImage.columns = ['항목', '빈도']
...
ws3 = wb.create_sheet(title="기관인상")
...
for row in dataframe_to_rows(disadvantages, index=False, header=True):
    ws2.append(row)
for row in dataframe_to_rows(institutionImage, index=False, header=True):
    ws3.append(row)    


# 엑셀 파일 저장
wb.save('한국보건산업진흥원_gpt분석_통계.xlsx')
